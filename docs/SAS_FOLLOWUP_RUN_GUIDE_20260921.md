...
- SAS 정적 점검(금지문·실패 중단 존재)과 Python 스키마 검사는 마쳤으나
  전체 SAS 실행 완료는 아직 확인하지 않았다. 첫 실행에서 확인된 범위와 수정은 아래와 같다.
...
최신 main을 Pull한 다음 새 SAS 세션에서 위 두 줄을 다시 실행한다.
앞선 실행의 outputs 폴더는 남겨 둔다. 새 실행 결과가 와야 전체 단계의
완료를 확인할 수 있다. 기존 실행이 성공했다고 표시하거나 오류 상태를
강제로 초기화해서 통과시키지 않는다.
