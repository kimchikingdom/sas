...
* **학습 조건 비교**: U5 동일 평가자료에서 선택 원문을 추가한 DUP 조건이 평균 FP 7.4건, FN 13.8건으로 오류 균형이 가장 좋았으나 통계적 우월을 확정할 수 없습니다.
* **취약 조건 규명**: DUP 기준 URL 포함 정상문자의 오탐률(2.01%)과 128토큰 초과 스미싱의 미탐률(16.84%)이 높게 관찰됐지만 단일 원인이나 인과효과로 단정할 수 없습니다.
* **개입 득실 검증**: KISA 7개 입력 조건 분석과 JEV 판정 대체·특징 결합 두 별도 실험에서 미탐 복구 시 정상 경보가 함께 늘거나 추가 이득이 없어, 관측 후 승자 교체 없이 기준 조건과 고정 임계값을 유지하기로 결정했습니다.
...
|---|---|---|
| U5 | 2,317건: 정상 1,665건, 스미싱 652건; 동일 자료 5개 seed (42, 101, 202, 303, 404) | 기존 시험·사후 진단 (과거 평가 이력이 있으며, 독립 1회 시험이 아님) |
| KISA | 고유 22건, 대표 18건 부분집합; 배송유형 양성 편의표본 | 양성 편의표본 (정상 문자가 없어 독립 정상 FPR 계산 불가) |
| 정상 후보 | 개발용 250건 | 경보 증가 점검용 (독립적인 정상 오탐률 표본이 아님) |
| JEV 대체 | seed 42에서 선택 88건; 오류 과대표집 | 사후 진단 (바꿨을 때 득실 확인용, 독립 시험이 아님) |
| JEV 결합 | 검증자료 재사용 142건 (정상 108건, 스미싱 34건) | 특징 결합 탐색 (추가 독립 이득을 입증하지 못함) |
...
아래 FP/FN은 동일 평가자료(2,317건)에서 5개 seed(42, 101, 202, 303, 404)의 결과를 평균낸 값입니다.
평균 건수의 소수점은 시드 평균의 표기이며 고유 문자 수가 아닙니다. Recall은 스미싱
분모(652건), FPR은 정상 분모(1,665건)를 기준으로 계산했습니다.

| 조건 | 평균 FP(건) | 평균 FN(건) | 평균 Recall | 평균 FPR |
|---|---:|---:|---:|---:|
| BASE | 10.0 | 14.2 | 97.82% | 0.60% |
| DUP | 7.4 | 13.8 | 97.88% | 0.44% |
| FLIP | 7.6 | 16.0 | 97.55% | 0.46% |

![U5 세 조건의 seed별 오류 분포](materials/20260921/diagnostics/u5/charts/error_distribution_arm.png)

*그림 A. U5 동일 평가자료에서 BASE·DUP·FLIP의 오류 분포. 색 막대는 평균 오류 건수, 점은 시드별 결과, 검은 오차막대는 5시드 모표준편차이며, 독립표본 신뢰구간이 아니다.*

### URL 포함과 입력 절단

근거: [최종 제출 집계의 URL·절단 항목](materials/20260921_final_v1/evidence/submission_final_20260921.json).

아래 본문 수치는 DUP 조건의 5개 시드 평균입니다.

URL이 있는 정상 집단은 정상 298건에서 평균 FPR 2.01%,
URL이 없는 집단은 정상 1,367건에서 0.10%였습니다.
이는 URL 포함 여부와 오탐의 연관 관찰이며, URL 자체가 오탐의 단일 원인이라는 증거가 아닙니다.
128토큰을 초과한 스미싱 57건의 FNR은 16.84%(평균 FN 9.6건),
한도 이내 595건은 0.71%(평균 FN 4.2건)이었습니다.
...
고유 KISA 22건(대표 부분집합 18건)은 배송 유형 중심의 양성 편의표본이며,
정상 후보 250건은 경보 증가를 점검하기 위한 별도의 개발용 자료입니다.
양성 표본에는 정상 문자가 없으므로 정상 오탐률을 계산할 수 없습니다.

기준 조건(`original128`)에서는 고유 22건 중 탐지 13건, 미탐 9건(탐지율 59.09%), 정상 후보 경보 11건이었습니다.
길이만 300토큰으로 늘린 `cap300` 단독 조건은 탐지 13건으로 미탐 복구가 전혀 없으면서 정상 후보 경보만 11건에서 24건으로 13건 증가했습니다.
반면 OCR 특수문자를 가리고 길이를 300토큰으로 확대한 `ocr_unk300` 조건에서는 탐지가 16건(72.73%)으로 증가했으나(잡음 삭제인 `ocr_delete300`도 14건으로 증가),
이 조건 역시 정상 후보 경보가 24건으로 늘어나는 득실이 관찰되었습니다.

사전 등록 후보였던 `head_tail128`(앞뒤를 합쳐 128토큰 유지)은 5개 시드 전체에서 미탐 복구 없이
새 경보가 발생해 불통과되었습니다. 기준 시드 42에서는 정상 후보 경보가 17건(신규 6건)으로
증가했습니다. 이에 따라 관측 후 승자 교체를 하지 않고 기존 조건(`original128`)과 고정 임계값을 유지하기로 결정했습니다.

| 입력 처리 | 탐지/미탐 | 탐지율 | 정상 후보 경보 | 새 경보 |
|---|---:|---:|---:|---:|
| `original128` | 13/9 | 59.09% | 11 | 0 |
| `ocr_delete128` | 13/9 | 59.09% | 11 | 0 |
| `cap300` | 13/9 | 59.09% | 24 | 13 |
| `ocr_delete300` | 14/8 | 63.64% | 24 | 13 |
| `ocr_unk128` | 13/9 | 59.09% | 11 | 0 |
| `ocr_unk300` | 16/6 | 72.73% | 24 | 13 |
| `head_tail128` | 13/9 | 59.09% | 17 | 6 |
...
첫째, 오류를 과대표집한 seed 42 선택 88건에서 JEV 판정으로 자동 대체한 결과, 기존 오탐 8건을
해소했지만 새로운 미탐 41건을 발생시키고 원래 정답이었던 1건이 판정 보류로 전환되어 채택하지 않았습니다.
둘째, 검증자료 142건을 재사용한 특징 결합 실험에서는 기준선(`rawK`)부터 오탐 0건, 미탐 0건이어서 결합의 추가 독립 이득을 입증할 수 없었습니다.

| 결합 조건 (기호) | 오탐 (FP) | 미탐 (FN) |
|---|---:|---:|
| KcBERT 기준선 (`rawK`) | 0건 | 0건 |
| KcBERT 점수만 재결합 (`K`) | 0건 | 0건 |
| KcBERT＋JEV (`KJ`) | 0건 | 0건 |
| KcBERT＋URL 특징 (`KU`) | 1건 | 0건 |
| KcBERT＋JEV＋URL 특징 (`KJU`) | 0건 | 0건 |
...
반환물 검수에 집중했습니다. 집계 반환 4단계(U5 진단, JEV 비교, JEV 결합, KISA 진단)를
전수 완료했습니다. 반환 ZIP의 55개 파일 중 CSV 집계를 검산하고, ODS 그래프 35개가 생성된 것을 확인했습니다.
