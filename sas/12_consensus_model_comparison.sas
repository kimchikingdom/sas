...
%macro sc12_defaults;
  ...
  %global sc12_n sc12_u sc12_bad sc12_bad2;
...
%macro sc12_input_check;
  %if not %sysfunc(fileexist(&sasdata./sas_consensus_predictions_20260914.csv)) or
      not %sysfunc(fileexist(&sasdata./sas_reviewer_ab_20260914.csv)) %then %do;
    %put ERROR: Missing consensus prediction or A/B reference input.; %abort cancel;
  %end;
%mend;
%sc12_input_check;
data work.sc12_p;
...
data work.sc12_review_ref;
  ...
  keep review_id final_category;
run;
%sc12_check;
proc sql noprint;
 select count(*),count(distinct catx('|',review_id,model,put(seed,best.),put(fold,best.))) into :sc12_n,:sc12_u from work.sc12_p;
  select count(*) into :sc12_bad from work.sc12_p where missing(review_id) or missing(model) or missing(seed) or missing(fold) or missing(role) or missing(original_label) or missing(final_category) or missing(probability) or missing(threshold) or missing(prediction) or prediction not in (0,1) or probability<0 or probability>1 or threshold<0 or threshold>1.000001 or final_category not in (1,2,3,4) or original_label not in ('normal','smishing') or role ne 'development_readout' or fold ne 0 or model not in ('LR-A','LR-B','LR-C','Kc-A','Kc-C','FAGE','Stacking');
 select count(*) into :sc12_bad2 from work.sc12_p where prediction ne (probability>=threshold);
quit;
%sc12_check;
%macro sc12_validate;
  %local sc12_bad3 sc12_bad4 sc12_bad5 sc12_bad6 sc12_bad7 sc12_bad8 sc12_ref_n sc12_ref_u sc12_ref_bad sc12_uid;
  %if &sc12_n ne 3122 or &sc12_u ne 3122 or &sc12_bad ne 0 or &sc12_bad2 ne 0 %then %do; %put ERROR: Invalid consensus prediction input rows=&sc12_n unique=&sc12_u bad=&sc12_bad threshold=&sc12_bad2.; %abort cancel; %end;
  proc sql noprint;
    select count(*),count(distinct review_id) into :sc12_ref_n,:sc12_ref_u from work.sc12_review_ref;
    select count(*) into :sc12_ref_bad from work.sc12_review_ref where missing(review_id) or missing(final_category) or final_category not in (1,2,3,4);
    select count(*) into :sc12_bad3 from (select seed,review_id from work.sc12_p group by seed,review_id having count(distinct model) ne 7);
    select count(*) into :sc12_bad4 from (select review_id,seed from work.sc12_p group by review_id,seed having count(distinct catx('|',original_label,put(final_category,best.))) ne 1);
    select count(*) into :sc12_bad5 from work.sc12_p where seed not in (42,101,202,303,404);
    select count(*) into :sc12_bad6 from (select seed,model,count(*) as nr from work.sc12_p group by seed,model having (seed=42 and nr ne 108) or (seed=101 and nr ne 88) or (seed=202 and nr ne 77) or (seed=303 and nr ne 71) or (seed=404 and nr ne 102));
    select count(*) into :sc12_bad7 from (select review_id from work.sc12_p group by review_id having count(distinct catx('|',original_label,put(final_category,best.))) ne 1);
    select count(*) into :sc12_bad8 from work.sc12_p p left join work.sc12_review_ref r on p.review_id=r.review_id where missing(r.review_id) or p.final_category ne r.final_category;
    select count(distinct review_id) into :sc12_uid from work.sc12_p;
  quit;
%sc12_check;
  %if &sc12_ref_n ne 556 or &sc12_ref_u ne 556 or &sc12_ref_bad ne 0 or &sc12_bad3 ne 0 or &sc12_bad4 ne 0 or &sc12_bad5 ne 0 or &sc12_bad6 ne 0 or &sc12_bad7 ne 0 or &sc12_bad8 ne 0 or &sc12_uid ne 325 %then %do; %put ERROR: Reference or model seed key metadata seed or row sets differ.; %abort cancel; %end;
%mend;
%sc12_validate;
